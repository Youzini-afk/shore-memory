...
class GroupChatDispatcher:
    ...
    async def decide_next_turn(self, room_id: str, history: List[GroupChatMessage]) -> Optional[str]:
        """
        决定下一个发言的角色。
        ...
        candidate_agents = [m.agent_id for m in members if m.agent_id not in ["user", "system", "Butler"]]
