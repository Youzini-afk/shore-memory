...
class StrongholdService:
    ...
    async def ensure_initial_data(self):
        ...
        if not facility:
            ...
            facility = await self.create_facility(
                name=default_facility_name, description="温馨的家", icon="HomeFilled"
            )
        ...
        for agent_id in all_config_agents:
            ...
            db_agent = (
                await self.session.exec(
                    select(AgentProfile).where(AgentProfile.name == agent_id)
                )
            ).first()
        ...
        all_agents = (
            await self.session.exec(
                select(AgentProfile).where(AgentProfile.role != "system")
            )
        ).all()
    ...
    async def create_room(
        ...
        if allowed_agents:
            for agent_id in allowed_agents:
                self.session.add(
                    GroupChatMember(room_id=room_id, agent_id=agent_id, role="member")
                )
    ...
    async def list_rooms(
        ...
        if facility_id:
            statement = select(StrongholdRoom).where(
                StrongholdRoom.facility_id == facility_id
            )
        else:
    ...
    async def update_room(
        self, room_id: str, updates: Dict[str, Any]
    ) -> Optional[StrongholdRoom]:
    ...
    async def get_room_agents(self, room_id: str) -> List[str]:
        statement = select(AgentLocation.agent_id).where(
            AgentLocation.room_id == room_id
        )
        return (await self.session.exec(statement)).all()
    ...
    async def process_butler_instruction(self, instruction: Dict[str, Any]) -> str:
        ...
        if action == "create_room":
            ...
            if not facility:
                facility = await self.create_facility(
                    name=facility_name, description="Auto-created facility"
                )
    ...
    async def process_butler_call(self, agent_id: str, query: str):
        ...
        context["persona"] = (
            persona_prompt.content if persona_prompt else "你是据点的 AI 管家。"
        )
