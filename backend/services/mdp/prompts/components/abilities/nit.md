...
- **循环终止 (CRITICAL)**:
  ...
  - **终止方法**: 调用 `finish_task(summary="任务完成的总结")`。
  - **警告**: 如果你不调用 `finish_task`，系统会认为你还需要继续操作，从而导致你陷入无意义的重复循环。
  - **状态更新**: 即使是 `update_status` 这种状态更新，执行后也必须决定是继续（调用其他工具）还是结束（调用 `finish_task`）。如果你只是想更新心情并结束，请在同一个脚本中先更新状态，最后调用 `finish_task`。

**示例 (更新心情并结束)**:
```nit
<nit-{{nit_id}}>
update_character_status(mood="happy")
finish_task(summary="我好开心呀！")
</nit-{{nit_id}}>
```
