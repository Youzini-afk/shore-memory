...
class PromptManager:
    ...
    def _generate_tools_description(self, tools: List[Dict[str, Any]]) -> str:
        """
        将 OpenAI Schema 格式所在的工具列表转换为 NIT 协议友好的自然语言描述。
    ...
    def _enrich_variables(
        ...
        if "available_tools_desc" not in variables:
            ...
            if dynamic_tools and isinstance(dynamic_tools, list):
                ...
                # 回退策略：如果未提供动态工具且不是社交模式，尝试从 PluginManager 全量获取
                # 但这通常意味着 AgentService 没有正确传递工具
                if not is_social_mode:
                    try:
                        from core.plugin_manager import get_plugin_manager

                        pm = get_plugin_manager()
                        all_tools = pm.get_all_tools_map()
                        if all_tools:
                            # 构造虚拟的 dynamic_tools 结构以复用生成逻辑
                            # 注意：这里我们缺乏详细的 description，只能用简单的名称
                            # 更佳做法是读取 manifest，但为保持简单，仅作为最后的兜底
                            tools_desc = "（系统警告：使用了全量工具列表作为兜底，可能包含未授权工具）\n"
                            tools_desc += ", ".join(all_tools.keys())
                            variables["available_tools_desc"] = tools_desc
                            logger.warning(
                                "[PromptManager] 使用了全量工具列表兜底，请检查 AgentService 是否正确传递了 dynamic_tools"
                            )
                        else:
                            variables["available_tools_desc"] = "（当前无可用工具）"
                    except Exception as e:
                        logger.error(f"[PromptManager] 生成兜底工具描述失败: {e}")
    ...
    def _generate_tools_description(self, tools: List[Dict[str, Any]]) -> str:
        """
        将 OpenAI Schema 格式的工具列表转换为 NIT 协议友好的自然语言描述。
        目标格式: - **tool_name**: 功能描述。参数 arg1: desc1; arg2: desc2。
        """
        if not tools:
            return "（当前无可用工具）"
    ...
    async def get_rendered_system_prompt(
        self, session: AsyncSession, is_social_mode: bool = False
    ) -> str:
        """
    ...
    def compose_messages(
        ...
        for msg in history:
            ...
            if msg.get("role") == "assistant":
                cleaned_msg["content"] = self.clean_history_for_api(
                    msg.get("content", "")
                )
