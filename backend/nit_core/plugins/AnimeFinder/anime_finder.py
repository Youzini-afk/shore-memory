...
async def find_anime_by_image(image_url: str, **kwargs) -> str:
    ...
    try:
        ...
        for i, res in enumerate(results, 1):
            ...
            formatted_output += f"**匹配 {i}** ({similarity:.1f}%)\n"
            formatted_output += (
                f"- **标题**: {native}" + (f" ({romaji})" if romaji else "") + "\n"
            )
            if english:
                formatted_output += f"- **英文名**: {english}\n"
            formatted_output += f"- **集数**: {episode}\n"
            formatted_output += (
                f"- **时间点**: {res.get('from', 0):.1f}s - {res.get('to', 0):.1f}s\n"
            )
            formatted_output += "\n"
