...
impl SocialMemoryEngine {
    ...
    fn add_memory(
        ...
        if content.contains("开心") {
            emotion |= SimHash::EMOTION_JOY;
        }

        let simhash =
            SimHash::compute_multimodal(&content, timestamp, emotion, SimHash::TYPE_EVENT);
    ...
    fn search(
        ...
        if let Some(q_vec) = query_vector {
            let mut final_results: Vec<(i64, f32)> = candidates
                .iter()
                .map(|(node, sim_score)| {
            ...
            // 仅使用 SimHash 分数
            let final_results: Vec<(i64, f32)> = candidates
                .iter()
