...
async def create_facility(name: str, description: str, icon: Optional[str] = None, session: AsyncSession = Depends(get_session)):
    service = StrongholdService(session)
    return await service.create_facility(name, description, icon)

@router.get("/rooms", response_model=List[StrongholdRoom])
async def list_rooms(facility_id: Optional[int] = None, session: AsyncSession = Depends(get_session)):
    service = StrongholdService(session)
    return await service.list_rooms(facility_id)

@router.post("/rooms", response_model=StrongholdRoom)
async def create_room(facility_id: int, name: str, description: str, session: AsyncSession = Depends(get_session)):
    service = StrongholdService(session)
...
async def update_butler_enabled(enabled: bool, session: AsyncSession = Depends(get_session)):
    service = StrongholdService(session)
    await service.update_butler_enabled(enabled)
    return {"status": "ok"}

@router.post("/butler/execute")
async def execute_butler_instruction(instruction: Dict[str, Any], session: AsyncSession = Depends(get_session)):
    service = StrongholdService(session)
...
async def get_all_agents_status(session: AsyncSession = Depends(get_session)):
    ...
    active_agents = (await session.exec(select(AgentProfile).where(AgentProfile.role != 'system'))).all()
