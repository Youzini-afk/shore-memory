...
async def default_prompts() -> dict[str, dict[str, str]]:
    ...
    return {
        "prompts": {
            ...
            "query_planner": _QUERY_PLANNER_SYSTEM_PROMPT,
        }
    }
...
@app.post("/v1/tasks/plan-query", response_model=PlanQueryResponse)
async def plan_query(req: PlanQueryRequest) -> PlanQueryResponse:
    query = req.query.strip()
    if not query:
        return PlanQueryResponse()

    client = OpenAICompatClient.from_role("query_planner")
    if client is None:
        return PlanQueryResponse()

    system_prompt = _resolve_system_prompt("query_planner", _QUERY_PLANNER_SYSTEM_PROMPT)
    data = await client.chat_json(system_prompt, query)
    if not isinstance(data, dict):
        return PlanQueryResponse()
    ...
    return PlanQueryResponse(subqueries=subqueries)


_QUERY_PLANNER_SYSTEM_PROMPT = """你是 shore-memory 的查询规划器。
你的任务是把一个用户检索 query 拆成适合记忆召回的 1-4 条子查询。

规则：
1. 如果原 query 只有一个明确意图，就只返回 1 条子查询。
2. 如果原 query 同时在问多个主题、对象、时间段或任务，拆成多条彼此独立的子查询。
3. 每条子查询都要自包含，尽量补全代词、省略和时间约束。
4. 保留原 query 里的专有名词、日期、地点、人名、品牌、作品名，不要泛化。
5. 不要扩写成答案，不要加入原 query 没有提到的事实。
6. 子查询要短、可直接用于 recall，避免重复表达。
7. 最多返回 4 条，按重要性排序。
