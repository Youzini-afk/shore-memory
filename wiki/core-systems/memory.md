...
- **PEDSA 算法 (Parallel Energy-Decay Spreading Activation)**:
  这是我们核心的加权图遍历算法。当一个记忆节点被检索命中（例如通过语义搜索或关键词锚点），搜索算法会沿着关联边（Edges）向四周进行加权传播。
...
当用户发送消息时，系统按以下步骤进行混合检索：

1.  **Anchor Position (锚点定位)**:
    - **Entity Matching**: 从用户输入中提取关键词，在 `Memory` 表中查找 `type="entity"` 的节点（如“小明”、“Python”）。
    - **Energy Injection**: 将这些实体节点赋予高初始能量（通常为 2.0），作为图扩散的“入口”。

2.  **Vector Recall (向量召回)**:
    - 使用 Embedding 对用户输入编码。
    - 从 VectorDB 召回 Top-20 语义相似的候选节点，注入初始能量。

3.  **Spreading (图扩散 - Rust)**:
    - 将所有能量注入 Rust `CognitiveGraphEngine`。
    - 执行 **PEDSA** 算法，在语义网络、思维簇和时间链表上进行 2 跳扩散。
    - 收集所有被激活节点的 ID 及其最终能量（GraphScore）。

4.  **Ranking (最终排序)**:
    - 应用 **混合评分公式**，结合 GraphScore、VectorScore、Importance 和对数衰减。
    - 过滤掉 `type="entity"` 节点（通常只返回事件和事实给 LLM）。
    - 截取 Top-K（通常为 5-10 个）。

5.  **Reconstruction (语境重构)**:
    - 根据选定记忆的 `prev_id` 和 `next_id` 自动拉取前后上下文。
    - 格式化为自然语言文本，注入到 Prompt。
...
class MemoryRelation(SQLModel, table=True):
    ...
    relation_type: str           # associative, causal, thematic, temporal, inhibitory
```
