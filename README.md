...
```
                                        ...
                                        └──────────┬──────────────────┘
                                                   │ 定时维护
                                                   ▼
  用户对话 ──→ Scorer ──→ save_memory() ──→ SQLite + 向量索引 + 图谱
                 │           │                     │
                 │           ├─ Embedding          │
                 │           ├─ 时间链表            │
                 │           └─ MemoryRelation     │
                 ...
                 │     ▼                   ▼
                 │  向量检索           图扩散传播 (PEDSA)
                 │  (IntentEngine)    (CognitiveGraphEngine)
                 │     │                   │
                 │     └───────┬───────────┘
                 │             ▼
                 │      Retrieval Enhancer
...
写入时同步完成三件事：
1. 计算语义 Embedding 向量，写入 `SemanticVectorIndex`
2. 维护时间链表（`prev_id` / `next_id`），保持记忆的时序连续性
3. 通过 LLM 分析 `MemoryRelation`（因果、关联、包含等），构建认知图谱边
...
| 阶段 | 算法 | 作用 | 实现 |
|:---|:---|:---|:---|
| ① 向量召回 | Cosine + HNSW | 从向量索引中快速找到语义相似的候选集 | Rust `IntentEngine` |
| ② 图扩散传播 | PEDSA + PPR | 从种子节点沿认知图谱传播激活能量，发现逻辑关联的远端记忆 | Rust `CognitiveGraphEngine` |
| ③ NMF 语义分析 | Lee & Seung, 1999 | 将候选集分解为隐含主题，评估查询的语义深度、主题覆盖度和新颖度 | Rust `RetrievalMath` |
| ④ FISTA 残差发现 | Beck & Teboulle, 2009 | 用稀疏编码检测"现有候选无法解释"的语义残差，触发二次检索发现弱信号 | Rust `RetrievalMath` |
| ⑤ 共现增益 | 统计关联 | 基于 Entity 共现频率增强关联记忆的排名 | Python |
| ⑥ DPP 多样性采样 | Kulesza & Taskar, 2012 | 用行列式点过程从候选集中选择质量最高且彼此最不相似的最终结果 | Rust `RetrievalMath` |

### 3. Rust 加速层

所有计算密集型算法均下沉至 Rust，通过 PyO3 暴露给 Python：

- **`CognitiveGraphEngine`**：动态类 CSR 邻接表，SmallVec 内联优化（边数 ≤ 4 零堆分配），Rayon 并行规约，u16 权重量化
- **`SemanticVectorIndex`** / `IntentEngine`：SIMD 友好的向量搜索，支持持久化到磁盘
- **`RetrievalMath`**：FISTA、DPP、NMF 三大算法的零拷贝 Rust 实现，NMF 支持 Agent 级缓存
