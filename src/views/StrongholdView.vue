<template>
  <div class="relative h-full w-full overflow-hidden">
    ...
    >
      ...
      <div v-else class="flex flex-col items-center justify-center h-full text-slate-400 relative z-10">
    ...
    >
      ...
      <div v-if="currentRoom" class="p-4 border-b border-slate-200 dark:border-slate-800">
        ...
        <div class="bg-slate-50 dark:bg-slate-800 rounded-lg p-3 text-xs font-mono break-all text-slate-600 dark:text-slate-300">
      ...
      <div class="flex-1 overflow-y-auto p-4 space-y-6">
        ...
        <div>
          <h4 class="text-xs font-bold text-slate-500 mb-3 flex justify-between items-center">
            本房间
            <span class="bg-green-100 dark:bg-green-900/30 text-green-600 dark:text-green-400 px-1.5 py-0.5 rounded text-[10px]">Active</span>
          </h4>
          <div class="space-y-3">
            <template v-if="currentRoomAgents.length > 0">
              <div v-for="agent in currentRoomAgents" :key="agent.name" class="flex items-center gap-3">
                <div class="w-8 h-8 rounded-full bg-slate-100 dark:bg-slate-800 flex items-center justify-center overflow-hidden border border-slate-200 dark:border-slate-700">
                   <!-- <img v-if="agent.avatar" :src="agent.avatar" class="w-full h-full object-cover" /> -->
                   <span class="text-xs font-bold text-slate-500">{{ agent.name[0].toUpperCase() }}</span>
                </div>
                <div>
                  <div class="text-sm font-medium text-slate-700 dark:text-slate-200">{{ agent.name }}</div>
        ...
        <div>
          <h4 class="text-xs font-bold text-slate-500 mb-3 flex justify-between items-center">
            据点全员
            <span class="bg-slate-100 dark:bg-slate-800 text-slate-500 px-1.5 py-0.5 rounded text-[10px]">Total</span>
          </h4>
          <div class="space-y-3">
            <div 
  ...
  <!-- Butler Call Modal -->
  <div v-if="showButlerModal" class="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm">
    <div class="bg-white dark:bg-slate-900 w-96 rounded-xl shadow-2xl border border-slate-200 dark:border-slate-700 overflow-hidden transform transition-all">
      <div class="p-4 border-b border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-800/50 flex justify-between items-center">
        ...
        <button @click="showButlerModal = false" class="text-slate-400 hover:text-slate-600 dark:hover:text-slate-200">
      ...
      <div class="p-4">
        ...
        <div class="flex justify-end mt-4 gap-2">
          ...
          >
            <component v-if="callingButler" :is="'Loading'" class="w-3 h-3 animate-spin" />
