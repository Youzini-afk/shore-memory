<template>
  ...
  <div class="h-full w-full flex overflow-hidden backdrop-blur-xl relative z-10">
    ...
    <div class="w-64 flex flex-col border-r border-white/40 bg-white/40 backdrop-blur-md pt-8">
      ...
      <div class="flex-1 overflow-y-auto custom-scrollbar px-2 space-y-1">
        ...
        <div
          ...
          :class="
            activeMode === 'direct' && activeAgentId === agent.id
        ...
        >
          <div class="relative">
            ...
            <div
              v-if="activeMode === 'direct' && activeAgentId === agent.id"
        ...
        <div
          class="px-2 py-1.5 text-[10px] font-bold text-slate-500/70 uppercase tracking-widest mt-4"
        >
          Groups
        </div>
        ...
        <div
          ...
          class="flex items-center gap-3 p-2 rounded-xl cursor-pointer transition-colors group"
          :class="
            activeMode === 'group' && activeGroupId === group.id
    ...
    <div class="flex-1 flex flex-col relative z-10 pt-8">
      ...
      <ChatInterface
        v-if="
          (activeMode === 'direct' && activeAgentId) || (activeMode === 'group' && activeGroupId)
        "
  ...
  >
    <div class="bg-white text-slate-900 rounded-2xl shadow-2xl w-96 p-6">
      ...
      <input
        ...
        placeholder="群聊名称"
        class="w-full mb-4 p-2 border rounded-lg bg-slate-50 text-slate-900 placeholder-slate-400 text-sm focus:outline-none focus:ring-2 focus:ring-sky-500/50"
      />
      ...
      <div class="mb-4 max-h-48 overflow-y-auto custom-scrollbar border rounded-lg p-2">
        ...
        <div
          v-for="agent in agents"
          :key="agent.id"
          class="flex items-center gap-2 mb-2 hover:bg-slate-50 p-1 rounded"
        >
          <input
      ...
      <div class="flex justify-end gap-2">
        <button
          class="px-4 py-2 text-slate-500 hover:bg-slate-100 rounded-lg text-sm font-medium"
          @click="showCreateGroup = false"
        >
          取消
        </button>
        <button
          class="px-4 py-2 bg-sky-500 text-white rounded-lg hover:bg-sky-600 text-sm font-bold shadow-lg shadow-sky-500/20"
          @click="createGroup"
        >
          创建
        </button>
